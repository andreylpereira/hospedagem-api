...
public class ReservaServiceImpl implements ReservaService {
    ...
    private ResponseEntity<?> validarDatas(ReservaDto reservaDto) {
        if (reservaDto.getDataInicio() == null || reservaDto.getDataFim() == null || reservaDto.getDataFim().isBefore(reservaDto.getDataInicio())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("O período de reserva não foi definido corretamente.");
        }
        return null;
    }
